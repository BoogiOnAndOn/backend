...
import com.boogionandon.backend.dto.ChangePasswordRequestDTO;
...
public class MemberController {
    ...
    public Map<String, Object> refresh(@RequestHeader("Authorization") String authHeader, @RequestParam(value = "refreshToken") String refreshToken) {
        ...
        String newRefreshToken =
                checkTime((Integer) claims.get("exp")) == true ? JWTUtil.generateToken(claims, 60 * 24 * 100)
                        : refreshToken;
    ...
    public Map<String, String> updateWorkerProfile(@PathVariable("workerId") Long workerId,
            @RequestBody WorkerUpdateDTO workerUpdateDTO) {
    ...
    public Map<String, String> updateAdminProfile(@PathVariable("adminId") Long adminId,
    ...
    // -------------- refresh 관련 시작 ------------------
    //--------------- 패스워드 변경(로그인 페이지의 비밀번호 찾기에서) 관련 시작 -------------
    ...
    //--------------- 패스워드 변경(로그인 페이지의 비밀번호 찾기에서) 관련 끝 -------------
    //--------------- 패스워드 변경(마이페이지 에서) 관련 시작 -------------
    
    @PatchMapping("change-password/{memberId}")
    public Map<String, String> changePassword(@PathVariable("memberId") Long memberId,
            @RequestBody ChangePasswordRequestDTO requestDTO) {
        
        try {
            memberService.confirmOldPassword(memberId, requestDTO.getOldPassword());
        } catch (PasswordMismatchException e) {
            return Map.of("oldError", e.getMessage());
        }
        
        try {
            memberService.changePassword(memberId, requestDTO.getNewPassword(), requestDTO.getNewConfirmPassword());
        } catch (CustomException e) {
            return Map.of("newError", e.getMessage());
        }
        
        return Map.of("result", "success");
    }
    //--------------- 패스워드 변경(마이페이지 에서) 관련 끝 -------------
