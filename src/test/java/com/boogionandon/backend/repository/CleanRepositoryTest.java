...
class CleanRepositoryTest {
    ...
    void testCleanInsert() {
        ...
        Admin managedAdmin = (Admin) memberRepository.findById(findCleaner.getManagerId())
                .orElseThrow(
                        () -> new NoSuchElementException("Admin with id " + findCleaner.getManagerId() + " not found"));
        ...
        Double beachLength = DistanceCalculator.calculateDistance(startLatitude, startLongitude, endLatitude,
                endLongitude);
    ...
    void testCleanInsert100() {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            Admin managedAdmin = (Admin) memberRepository.findById(randomCleaner.getManagerId())
                    .orElseThrow(() -> new NoSuchElementException(
                            "Admin with id " + randomCleaner.getManagerId() + " not found"));
            ...
            List<String> memberList = cleaner.stream()
                    .filter(member -> member.getManagerId().equals(randomCleaner.getManagerId()) && !member.equals(
                            randomCleaner))
...
//      Page<Clean> byStatusNeededAndSearchForSuper = cleanRepository.findByStatusNeededAndSearchForSuper(beachSearch, pageable);
            Page<Clean> byStatusNeededAndSearchForSuper = cleanRepository.findByStatusNeededAndSearchForSuper("",
                    pageable);
...
//      Page<Clean> byStatusNeededAndSearchForRegular = cleanRepository.findByStatusNeededAndSearchForRegular(beachSearch, pageable, adminId);
            Page<Clean> byStatusNeededAndSearchForRegular = cleanRepository.findByStatusNeededAndSearchForRegular("",
                    pageable, adminId);
...
//      Page<Clean> byStatusCompletedAndSearchForSuper = cleanRepository.findByStatusCompletedAndSearchForSuper(beachSearch, pageable);
            Page<Clean> byStatusCompletedAndSearchForSuper = cleanRepository.findByStatusCompletedAndSearchForSuper("",
                    pageable);
...
//      Page<Clean> byStatusCompletedAndSearchForRegular = cleanRepository.findByStatusCompletedAndSearchForRegular(beachSearch, pageable, adminId);
            Page<Clean> byStatusCompletedAndSearchForRegular = cleanRepository.findByStatusCompletedAndSearchForRegular(
                    "", pageable, adminId);
