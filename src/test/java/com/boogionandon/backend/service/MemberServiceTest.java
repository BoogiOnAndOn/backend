...
class MemberServiceTest {
    ...
    @Test
    @DisplayName("softDeleteMembers 테스트")
    @Commit
    void softDeleteMembers() {
        List<Long> ids = Arrays.asList(31L, 32L, 33L); // initData 에 의해 자동저장된
        
        BulkDeleteRequestDTO build = BulkDeleteRequestDTO.builder()
                .ids(ids)
                .build();
        
        memberService.softDeleteMembers(build);
